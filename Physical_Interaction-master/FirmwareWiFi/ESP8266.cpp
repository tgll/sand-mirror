...
ESP8266::ESP8266(int pinRx, int pinTx)
{
	m_puart = new SoftwareSerial(pinRx, pinTx);
}
#else
ESP8266::ESP8266(): m_puart(&Serial1) // hard coded to use Serial1 (only works on board with two or more hardware serial ports)
{
...
bool ESP8266::init(const char* ssid, const char* pwd, uint32_t baudRateSet)
{
	if (!initSequence(ssid, pwd))
	{
...
bool ESP8266::initSequence(const char* ssid, const char* pwd, uint32_t baudRateSet)
{
	Serial.println(F("Initializing device..."));
	if (autoSetBaud(baudRateSet))
	{
		Serial.println(F("Baudrate set success"));
	}
	else
	{
		Serial.println(F("Baudrate set failed"));
		return false;
	}
	...
	{
		Serial.println(F("Station + softAP - OK"));
	}
	else
	{
		Serial.println(F("Station + softAP - Error, Reset Board!"));
		return false;
	}

	if (joinAP(ssid, pwd))
	{
		Serial.print(F("Joining AP successful, "));
		Serial.println(getLocalIP().c_str());
	}
	else
	{
		Serial.println(F("Join AP failure, Reset Board!"));
		return false;
	}

	if (disableMUX())
	{
		Serial.println(F("Single Mode OK"));
	}
	else
	{
		Serial.println(F("Single Mode Error, Reset Board!"));
		return false;
	}
	return true;
...
bool ESP8266::sendSingle(const char* header, const char* body, const char* footer)
{
	rx_empty();
	m_puart->print("AT+CIPSEND=");
	m_puart->println(strnlen(body, 500) + strlen(header) +  strlen(footer));
	if (recvFind(">", 500)) {
		rx_empty();
		m_puart->print(header);
		m_puart->print(body);
		m_puart->println(footer);
	...
	while (millis() - start < 1500) {
		while (m_puart->available() > 0 && i < bufferLen)
		{
			...
			if(c == '{'&& bodyFlag)
				{
					bodyFlag =0;
					char *codeIdx = strstr (buffer,"200 OK");
					if(codeIdx != NULL)
					{
						Serial.println(F("Code 200 OK"));
						i=0;
					}
					else
					{
						Serial.println(F("Code Error!"));
						return 0;
					}
				}
			else if (c == '}')
...
int ESP8266::httpGet(const char* header, const char* body, const char* footer, char* buffer, const int maxBufferSize)
{
	rx_empty();
	if (createTCP("proxy.circuito.io", 80))
	{
	...
	if (!sendSingle(header, body, footer))
	{
		Serial.print(F("not sent"));
		//return "";
	}

	int len = recvSingle(buffer, maxBufferSize);
